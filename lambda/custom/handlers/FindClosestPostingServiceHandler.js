...
const findClosestPostingServiceHandler = Alexa.CreateStateHandler(config.APP_STATES.FIND_POSTING_SERVICE, {
    async PostingServiceIntent() {
        ...
        try {
            ...
            console.log("user address request: " + userAddressRequest)
            const userAddress = userAddressRequest.data;
            if (userAddress.addressLine1 == null || userAddress.addressLine1 == "") ResponseHelper.sendResponse(alexa, alexa.t("NO_ADDRESS"))
            ...
            if(err.response.status == 403) ResponseHelper.sendResponse(alexa, alexa.t("NOTIFY_MISSING_PERMISSIONS"));
        }
